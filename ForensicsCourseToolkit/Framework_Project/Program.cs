...
namespace ForensicsCourseToolkit
{
    public static class Program
    {
        ...
        private static void TestLibraries(bool testCompression = true, bool testEncryption = true)
        {
            ...
            var myOrgMsg = "123456";
            var encryptedx = Framework_Project.Crypto.AESGCM.SimpleEncryptWithPassword(myOrgMsg, "123456");

            var decryptedx = Framework_Project.Crypto.AESGCM.SimpleDecryptWithPassword(encryptedx, "1234567");
            try
            {
                Parallel.For(0, 1000000, i =>
                   {
                     ...
                     //  string decompressed = Framework_Project.Compression.CompressionHelper.Unzip(compressedString);
                       ...
                       string encrypted = Framework_Project.Crypto.AESGCM.SimpleEncryptWithPassword(compressedString, $"Keyyy{i}");
                       if (encrypted == null)
                       {
                           throw new Exception("encrypted==null");
                       }

                       string decrypted = Framework_Project.Crypto.AESGCM.SimpleDecryptWithPassword(encrypted, $"Keyyy{i}");
                       if (decrypted == null)
                       {
