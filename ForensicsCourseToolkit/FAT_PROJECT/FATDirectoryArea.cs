...
namespace ForensicsCourseToolkit.Filesystems
{
    ...
    public class FatDirectoryArea : IHaveStartAddress, IHaveStructure, IHaveExactSize,IHaveCommonProps
    {
        ...
        public int Size { get; } = Common.FatDirectoryAreaSizeBytes;
        ...
        {
            return Helpers.Conversion.HexLittleEndianToInt(Structure[11].Value);
        }

        public int GetDataLocation()
        {
            int rootDirectoryStartSector = (RootDirectoryStartAddress/Common.BootSectorSizeBytes);
            int maxRootDirectoriesSectors = (MaxNumofRootDirectories*32/Common.BlockSizeInBytes);
            int val= (rootDirectoryStartSector + maxRootDirectoriesSectors + SectorsPerCluster * (GetFirstClusterAddress() - 2)) * Common.BlockSizeInBytes;
