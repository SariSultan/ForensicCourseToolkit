...
namespace ForensicsCourseToolkit.Helpers
{
    public static class Printer
    {
        ...
        private static readonly int padRightLarge = 50;
        private static readonly int parallelLineLength = 110;
        ...
        public static void PrintVbrListWithNTFSBootSectors(string fileName, ref List<Vbr> vbrList, ref Logger aLogger, ref RichTextBox rtb, bool printHex)
        {
            foreach (var vbr in vbrList)
            {
                var bs = Common.GetUnit<NTFSBootSector>(fileName, ref aLogger, vbr.BootSectorStartLocationByte(), $"Boot sector {vbr.Description}", null);
                if (vbr.IsEmptyPartition())
                {
                    rtb.AppendText($"Printing {bs.Description}", Color.DarkCyan, true);
                }
